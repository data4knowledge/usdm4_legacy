...
class CleanHTML:
                        ...
                        if hasattr(current_element, 'name'):
                            ...
                            if current_element.name in ["h1", "h2", "h3", "h4", "h5", "h6"]:
                                ...
                                if (not found_toc_tables and 
                                    (len(heading_text) > 50 or  # Likely a repeated document title
                                     "LIST OF" in heading_text.upper() or
                                     "APPENDICES" in heading_text.upper())):
                            ...
                            # Stop if we hit actual content (paragraphs with substantial text)
                                ...
                                if len(text) > 100 and not self._looks_like_toc_content(text):
                                    break
    ...
    def _is_toc_table(self, table):
        ...
        try:
            ...
            toc_indicators = [
                ...
                "protocol", "synopsis", "objective", "endpoint",
                "study design", "inclusion", "exclusion", "assessment",
                "statistical", "safety", "efficacy", "analysis"
            ]
            
            # Count how many TOC indicators are present
            indicator_count = sum(1 for indicator in toc_indicators if indicator in table_text)
            ...
            # Decision logic
            return (indicator_count >= 3 or  # Many TOC-related terms
                   (indicator_count >= 1 and len(page_numbers) >= 3) or  # Some indicators + page numbers
                   (has_toc_structure and indicator_count >= 1) or  # Good structure + some indicators
                   (has_toc_structure and len(page_numbers) >= 2))  # Good structure + some page numbers
    ...
    def _looks_like_toc_content(self, text):
        ...
        return any(phrase in text_lower for phrase in [
            ...
            "list of appendices", "page", "section"
        ])
    ...
    def _get_soup(self, text: str) -> BeautifulSoup:
        try:
            ...
            self._errors.exception(
                f"Parsing '{text}' with soup", e, KlassMethodLocation(self.MODULE, "get_soup")
            )
