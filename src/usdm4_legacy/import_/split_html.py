...
class SplitHTML:
    ...
    def process(self) -> list[dict] | None:
        ...
        try:
            ...
            self._errors.exception(f"Exception raised splitting document into sections", e, location)
            return None
    ...
    def _parse_section_heading(self, heading) -> dict | None:
        ...
        try:
            ...
            if match:
                section_number = match.group(1).rstrip('.')  # Remove trailing dot if present
            ...
            while current_element:
                ...
                if hasattr(current_element, 'name') and current_element.name:
                    ...
                    if (current_element.name in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'] and 
                        self._parse_section_heading(current_element)):
                        break
    ...
    def _get_soup(self, text: str) -> BeautifulSoup:
        ...
        try:
            ...
            self._errors.exception(
                f"Parsing HTML with soup", e, KlassMethodLocation(self.MODULE, "_get_soup")
            )
