...
def save_sections_to_yaml(sections, output_filename):
    ...
    try:
        ...
        with open(output_path, "w", encoding="utf-8") as file:
            yaml.dump(
                sections, file, default_flow_style=False, allow_unicode=True, indent=2
            )
...
def test_file(filename):
    ...
    if sections:
        ...
        for i, section in enumerate(sections[:10]):  # Show first 10 sections
            ...
            print(
                f"{i + 1:2d}. Section {section['section_number']}: {section['section_title'][:50]}{'...' if len(section['section_title']) > 50 else ''}"
            )
        ...
        if sections:
            ...
            print(
                f"Content preview: {sections[0]['html_content'][:200]}{'...' if len(sections[0]['html_content']) > 200 else ''}"
            )
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if sections:
            ...
            for section in sections:
                ...
                print(
                    f"    Content: {section['html_content'][:100]}{'...' if len(section['html_content']) > 100 else ''}"
                )
        else:
